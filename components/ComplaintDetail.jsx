...
export const ComplaintDetail = ({ complaint, onBack, onVote, onAddComment }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 sm:p-6 md:p-8">
      <button
        onClick={onBack}
        className="flex items-center space-x-2 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-semibold mb-6"
      >
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center border-b dark:border-slate-700 pb-4 mb-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-300">
            Complaint against <span className="font-semibold">{complaint.vendorName}</span> on Train{' '}
            <span className="font-semibold">{complaint.trainNo}</span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-8">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed bg-slate-50 dark:bg-slate-900/50 p-4 rounded-md">
          ...
          {complaint.evidenceUrl && (
            <div>
              ...
              <img
                ...
                alt="Evidence for complaint"
                className="rounded-lg shadow-md max-w-full h-auto border dark:border-slate-700"
              />
            </div>
          )}
          ...
          <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg">
              {complaint.comments.map((comment) => (
                <div
                  key={comment.id}
                  className="border-b border-slate-200 dark:border-slate-700 py-3 last:border-b-0"
                >
                  <p className="text-slate-800 dark:text-slate-200">{comment.text}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    by {comment.user.name} on {new Date(comment.timestamp).toLocaleDateString()}
                  </p>
                </div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <div>
                ...
                <div className="flex items-center pt-2">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-800 dark:text-slate-100">
                      {complaint.geolocation.lat.toFixed(4)}, {complaint.geolocation.lng.toFixed(4)}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="text-sm text-slate-500 dark:text-slate-400 pt-2">
              <p>
                Filed by: <span className="font-medium text-slate-600 dark:text-slate-300">{complaint.user.name}</span>
              </p>
              <p>
                Date:{' '}
          ...
          <div>
            ...
            <div className="space-y-3">
              {complaint.history.map((historyItem, index) => (
                <div key={index} className="flex items-start text-sm">
                  <div className={`w-2.5 h-2.5 rounded-full mt-1.5 mr-3 border-2 ${statusStyles[historyItem.status]}`}></div>
                  <div>
                    <p className="font-semibold text-slate-800 dark:text-slate-100">{historyItem.status}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      {new Date(historyItem.timestamp).toLocaleString()}
                    </p>
                    {historyItem.notes && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 italic mt-1">"{historyItem.notes}"</p>
                    )}
                  </div>
                </div>
              ))}
...
ComplaintDetail.propTypes = {
  complaint: PropTypes.shape({
    ...
    trainNo: PropTypes.string.isRequired,
    vendorName: PropTypes.string.isRequired,
    itemName: PropTypes.string.isRequired,
    reportedPrice: PropTypes.number.isRequired,
    ...
    history: PropTypes.arrayOf(historyShape).isRequired,
